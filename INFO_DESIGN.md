...
infers rather than reports. It rests on an absence -- the wheels that were
*not* heard -- so it declines often: nothing labelled, both sides heard at
similar strength, or no levels to compare all produce an empty cell. A reading
nothing contradicts is shown firm; one resting on a level comparison or on
wheels whose side is unknown carries a "?" and a quieter style. Every reading
puts its basis in the tooltip, so it can be checked rather than taken as a
measurement.
...
there. Direction is computed at read time from the wheel labels as they are
now, never stored: a stored guess goes stale the moment someone corrects a
wheel.
...
pins the sensor, and anything that pins must also be un-pinnable from the UI.
Every action that pins says so in the flash it returns -- "moved to X and
pinned there", not just the move -- because otherwise the sensor comes back
wearing a pill the user was never told about.
...
The bar appears on the first tick and wears the accent of the ticked rows,
rather than standing there permanently under a label reading "Tick the sensors
to act on:". The checkbox column already says the rows can be ticked, and the
shared accent is what shows which rows the bar acts on. It is hidden from
script and rendered either way, so with JS off every action stays reachable.

**A bar of actions sits inside the set it acts on.** The ticks, the table and
the buttons that read them share one panel, with the bar as its footer. As a
sibling it had no ground of its own and sat equidistant between two panels,
reading as the heading of the wrong one.
...
and the selection lost. A rejected mutation redirects back with the reason,
and the flash records whether it is an outcome or a refusal, so a turned-down
action is not coloured like a save. Only a request that asked for JSON gets a
400.
...
destroys something. Eleven identical grey buttons in one region gives no
hierarchy at all.
...
box was clicked, and what you clicked ("front left") was not what it stored
("FL"), so the control appeared to change the answer after it was given. A
picker with one field saves on change; the "Set" button beside it is the
no-script fallback, hidden by the script that replaces it. A label already
stored outside the set is kept as an option of its own, so opening a page
never offers to erase data. And because a car has
...
filter toggles on `/vehicles`, so the tiles are the worklist. Without a way to
gather them, the flags are not actionable.

**Two scales never share a plot.** Where two y-axes line up is arbitrary, so
a correlation read off that alignment is not in the data. Use stacked facets
with a synced cursor. Every chart also carries a table view.
...
in the README, not on the Sensors page. Prose on screen has three jobs — warn
before something irreversible, state a truncation, tell an empty page what to
do next — and a control that names its own action does not also get a
paragraph telling you to press it. Every `<h1>` and most `<h2>`s once carried a paragraph, and several
...
`rssi_margin` were once locals bound when the app was built, which stopped
working when the Settings page could change a value: the save reached the file
and the service but not the pages. The template globals are callables
(`{{ timezone() }}`) rather than values, because a Jinja macro cannot see the
render context, so a called global is what works from inside `_macros.html`
...
a save leaves a config that fails to load on the next start, from a UI that
reported success. A refusal names the box it came from -- there are forty on
that page, and "invalid literal for int()" identifies none of them -- and
leaves the file byte-identical.

**A rewritten config still explains itself.** The Settings page rewrites
`config.yaml` in place, so the explanatory prose lives beside the defaults in
`config.py` and is re-emitted on every save. The previous version is kept as
`.bak`, since comments regenerated from code cannot recover a note someone
typed by hand.

**A setting the running process cannot adopt is shown, not offered.** Only
`database` is in that set: every component holds a connection to the file
opened at startup, so a box for it would change where the next run looks and
nothing about this one. It renders as text rather than as a control that
...
service restarts, since a reminder that vanishes on the next click leaves the
setting silently not in force. Restarting is an exec of this same program
rather than an exit: exiting would be a restart under systemd and a shutdown
in a terminal.
